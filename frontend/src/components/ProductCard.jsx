  ...
  return (
    <Link to={`/product/${p.id}`} className="product-card group bg-white rounded-2xl border border-[#eae5d8] overflow-hidden block">
      <div className="relative aspect-square bg-[#f5f0e6] overflow-hidden">
        ...
        {p.badge && <span className="absolute top-3 left-3 bg-[#2e7d32] text-white text-[10px] uppercase tracking-widest font-bold px-2.5 py-1 rounded-full">{p.badge}</span>}
        {p.oldPrice && <span className="absolute top-3 right-3 bg-[#c73e1d] text-white text-[10px] uppercase tracking-widest font-bold px-2.5 py-1 rounded-full">-{Math.round((1 - p.price/p.oldPrice)*100)}%</span>}
        <div className="qview absolute bottom-3 left-3 right-3 flex gap-2">
          <button onClick={handleAdd} className="flex-1 bg-white text-[#1f2a1a] text-xs font-semibold py-2.5 rounded-full flex items-center justify-center gap-1 hover:bg-[#2e7d32] hover:text-white transition"><Plus size={14} /> Add</button>
          <span className="w-10 h-10 bg-white rounded-full flex items-center justify-center hover:bg-[#2e7d32] hover:text-white transition"><Eye size={14} /></span>
        </div>
      </div>
      <div className="p-4">
