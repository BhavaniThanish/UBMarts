...
const CartDrawer = () => {
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 right-0 h-100 w-100 sm:w-[420px] bg-[#faf7f2] z-[70] shadow-2xl transition-transform duration-500 d-flex flex-column ${isOpen ? "translate-x-0" : "translate-x-full"}`}>
        <div className="d-flex align-items-center justify-content-between p-5 border-b border-[#e8e2d4]">
          <h3 className="font-serif text-2xl text-[#1f2a1a] d-flex align-items-center gap-2"><ShoppingBag size={20} /> Your Basket</h3>
          <button onClick={() => setIsOpen(false)} className="p-2 hover:bg-[#e8e2d4] rounded-circle" aria-label="close cart"><X size={18} /></button>
        </div>
        <div className="flex-1 overflow-y-auto p-5">
          {items.length === 0 ? (
            <div className="h-100 d-flex flex-column align-items-center justify-content-center text-center gap-4">
              <div className="w-20 h-20 rounded-circle bg-[#e8e2d4] d-flex align-items-center justify-content-center"><ShoppingBag size={30} className="text-[#4a7c2a]" /></div>
              <div>
            ...
            <div className="space-y-4">
              {items.map(it => (
                <div key={it.id} className="d-flex gap-3 bg-white p-3 rounded-xl border border-[#e8e2d4]">
                  ...
                  <div className="flex-1">
                    ...
                    <div className="mt-2 d-flex align-items-center justify-content-between">
                      <div className="d-flex align-items-center border border-[#e8e2d4] rounded-circle">
        ...
        {items.length > 0 && (
          <div className="border-t border-[#e8e2d4] p-5 bg-white">
            ...
            <button className="w-100 btn-primary justify-content-center">Proceed to Checkout</button>
            <button onClick={clear} className="w-100 mt-2 text-xs text-[#6b7360] hover:text-red-600 py-2">Clear basket</button>
          </div>
        )}
      </aside>
    </>
